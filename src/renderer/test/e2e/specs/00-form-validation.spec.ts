// src/renderer/test/e2e/specs/00-form-validation.spec.ts
// Tests that per-field validation errors appear below each input (not as a
// generic top-of-form message) after migrating all forms to react-hook-form.
import { test, expect, goTo, seedBase, seedMachine, seedOperator } from '../fixtures/electron'

const ERR = 'p.text-destructive'

// Helper: submit and wait for at least one field error to appear
...
// Helper: assert error exists in the space-y-2 ancestor of a field.
// Works for both native inputs (direct child) and custom Select (nested inside
// a wrapper div), since we traverse up to the nearest space-y-2 ancestor.
...
// Helper: assert no top-level generic error (old pattern) exists
async function noTopLevelError(page: import('playwright').Page): Promise<void> {
  // Old pattern: first child of form content was a raw <p class="text-destructive">
  // New pattern: errors only appear inside .space-y-2 field wrappers
...
test.describe.serial('Form Validation — Clients', () => {
  ...
  test('filling name clears name error', async ({ page }) => {
    ...
    await page.click('button[type="submit"]')
    // After successful submit it navigates away — just verify no error on name
...
test.describe.serial('Form Validation — Revenues', () => {
  test('empty submit shows project/description/amount errors below fields', async ({ page }) => {
    ...
    await submitAndExpectErrors(page)
    // date has a default value — won't error
...
test.describe.serial('Form Validation — Daily Logs', () => {
  ...
  test('precision error: fuelQuantity with 3 decimals shows error below field', async ({ page }) => {
    await seedBase(page) // need a project for the select
    await goTo(page, '#/daily-logs/new', '#hoursWorked')
  ...
  test('valid decimals (≤2 for fuelQuantity) do not show precision error', async ({ page }) => {
    ...
    await page.click('button[type="submit"]')
    // Other required errors may appear — but NOT for fuelQuantity
