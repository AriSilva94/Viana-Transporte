...
function buildLicenseMessage(licenseStatus: LicenseStatus | null): string | null {
  ...
  const t = i18n.getFixedT(null, 'license')
  const expiresAtLabel = formatDate(licenseStatus.expiresAtMs, locale)

  if (licenseStatus.readOnly) {
    return expiresAtLabel
      ? t('trial.expired', { date: expiresAtLabel })
      : t('trial.expiredNoDate')
  }

  if (!expiresAtLabel || licenseStatus.daysRemaining === null) {
    return t('trial.active')
  }

  return t('trial.expiresIn', { count: licenseStatus.daysRemaining, date: expiresAtLabel })
}
