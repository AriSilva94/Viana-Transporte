  ...
  return (
    <DropdownMenu.Root open={open} onOpenChange={setOpen} modal={false}>
      ...
      <DropdownMenu.Portal>
        ...
        >
          <DropdownMenu.Item
            onSelect={() => void handle(onExportExcel)}
            className="flex w-full items-center gap-2.5 px-3 py-2.5 text-left text-sm text-foreground cursor-pointer outline-none transition-colors data-[highlighted]:bg-brand-sand/15"
          >
            <FileSpreadsheet className="h-4 w-4 shrink-0 text-green-600" />
            {t('reports:export.excel')}
          </DropdownMenu.Item>
          <DropdownMenu.Item
            onSelect={() => void handle(onExportPdf)}
            className="flex w-full items-center gap-2.5 px-3 py-2.5 text-left text-sm text-foreground cursor-pointer outline-none transition-colors data-[highlighted]:bg-brand-sand/15"
          >
