...
export function extractDeepLinkUrlFromArgv(argv: string[], protocol = 'viana-transporte'): string | null {
...
function enqueueOrDeliverUrl(getAuthService: () => AuthService | null, pendingUrls: string[], url: string): void {
  ...
  if (authService) {
    void authService.handleCallbackUrl(url).then(notifyRenderer)
    return
  }
...
export function createAuthDeepLinkRuntime(): AuthDeepLinkRuntime {
  ...
  if (process.platform === 'win32' && !app.isPackaged) {
    app.setAsDefaultProtocolClient('viana-transporte', process.execPath, [resolve(process.argv[1])])
  } else {
